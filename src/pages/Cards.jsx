...
export default function Cards() {
  ...
  function add(e) {
    ...
    if (!cardItem) return alert("Sem cartão base no seed.");
    if (!desc.trim() || !v) return alert("Descrição e valor obrigatórios.");
  ...
  return (
    <div style={card}>
      ...
      <form onSubmit={add} style={{ display: "grid", gridTemplateColumns: "160px 1fr 160px 160px", gap: 10, marginTop: 16 }}>
      ...
      <div style={{ marginTop: 16, display: "grid", gap: 10 }}>
        {(db.cardPurchases || []).filter(p => p.cardId === cardItem?.id).map(p => (
          <div
            key={p.id}
            style={{
              padding: 12,
